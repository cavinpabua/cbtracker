...
let hideAddress = (localStorage.getItem('hideAddress') === 'true');
let currCurrency = localStorage.getItem('currency');
const currencies = ['php', 'aed', 'ars', 'aud', 'brl', 'cny', 'eur', 'gbp', 'hkd', 'idr', 'inr', 'jpy', 'myr', 'sgd', 'thb', 'twd', 'usd', 'ves', 'vnd'];
let includeClaimTax = (localStorage.getItem('includeClaimTax') === 'true');
let rewardsClaimTaxMax = 0;
let storeAccounts = [];
let storeNames = {};
let skillPrice = 0;
let localPrice = 0;
let bnbPrice = 0;
let usdPrice = 0;
const $table = $('#table-accounts tbody');
...
async function refreshTimer() {
  ...
  function incTimer(){
    setTimeout (function(){
      if(countdownNum !== 0){
        countdownNum--;
        document.getElementById('btn-timer').innerHTML = `Refresh again in ${countdownNum} seconds`;
        incTimer();
      } else {
...
async function oracleTicker() {
  const oraclePrice = 1 / fromEther(`${await getOraclePrice()}`);
...
async function loadData () {
  ...
  const fRowHtml = await Promise.all(storeAccounts.map(async (address, i) => {
    ...
    let charCount = parseInt($cardChar.html());
    charCount += charIds.length
    $cardChar.html(charCount)

    let charLen = charIds.length;
...
function addAccount() {
  ...
  const address = $('#inp-address').val().trim();
...
function renameAccount() {
  const name = $('#inp-rename').val().trim();
  const address = $('#inp-readdress').val().trim();
...
function sortTable() {
  ...
  const rows = $table.children().detach().get();
